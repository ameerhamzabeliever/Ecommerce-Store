...
extension UIViewController{
    ...
    func showAlert(title: String,description: String,cancelBtnHide: Bool = true,alertType: AlertType = .General,delegate: StoreHelpVCDelegates?){
        guard let alertVC = R.storyboard.storeSB.storeHelpVC() else {return}
        alertVC.currentTitle = title
        alertVC.currentDescription = description
        alertVC.delegate = delegate
        alertVC.isCancelBtnHide = cancelBtnHide
        alertVC.type = alertType
        self.present(alertVC, animated: true)
    }
    func showAlert(title: String, errorMessages: [String]?,cancelBtnHide: Bool = true,alertType: AlertType = .General) {
        guard let alertVC = R.storyboard.storeSB.storeHelpVC() else {return}
        alertVC.currentTitle = title
        alertVC.errorDescriptions = errorMessages
        alertVC.isCancelBtnHide = cancelBtnHide
        alertVC.type = alertType
        self.present(alertVC, animated: true)
    }
...
extension UIColor {
    ...
    static let appBorderRedColor = UIColor.init(red: 235/255, green: 87/255, blue: 87/255, alpha: 1)
}
...
extension UITextField {
    ...
    func setInputViewDatePicker(target: Any, selector: Selector) {
        ...
        let cancel = UIBarButtonItem(title: PisiffikStrings.cancel(), style: .plain, target: nil, action: #selector(tapCancel)) // 6
        let barButton = UIBarButtonItem(title: PisiffikStrings.done(), style: .plain, target: target, action: selector) //7
