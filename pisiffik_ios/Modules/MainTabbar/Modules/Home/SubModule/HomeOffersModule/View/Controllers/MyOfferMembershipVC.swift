...
class MyOfferMembershipVC: BaseVC {
...
extension MyOfferMembershipVC: AllOffersViewModelDelegate{
    ...
    func didReceiveServer(error: [String]?, type: String, indexPath: Int) {
        ...
        if type == APIType.homeAPI{
            ...
            self.itemsCollectionView.setEmptyDataSet(title: error?.first ?? "", image: R.image.ic_sometong_went_wrong_image() ?? UIImage(), buttonTitle: PisiffikStrings.tryAgain(), shouldDisplay: true) {
                self.getAllOffers(of: .memberships)
            }
        }
    }
...
extension MyOfferMembershipVC: RemoveFromFavoriteViewModelDelegate{
...
extension MyOfferMembershipVC: AddPisiffikItemToCartViewModelDelegate{
...
extension MyOfferMembershipVC: PisiffikItemDetailProtocol{
...
extension MyOfferMembershipVC: AddToFavoriteViewModelDelegate{
...
extension MyOfferMembershipVC: CollectionViewMethods{
        ...
        case itemsCollectionView:
            ...
            if gridBtn.isSelected{
                if UIDevice().userInterfaceIdiom == .phone{
                    return CGSize(width: width / 2, height: 264.0)
                }else{
...
extension MyOfferMembershipVC{
    ...
    private func configureListCell(at indexPath: IndexPath) -> UICollectionViewCell{
        let cell = self.itemsCollectionView.dequeueReusableCell(withReuseIdentifier: .favoritesListCell, for: indexPath) as! FavoritesListCell
...
//            }
    ...
    private func configureGridCell(at indexPath: IndexPath) -> UICollectionViewCell{
        let cell = self.itemsCollectionView.dequeueReusableCell(withReuseIdentifier: .favoritesGridCell, for: indexPath) as! FavoritesGridCell
